...
typedef enum {false,true} Boolean;
 ...
 struct gestore_t {
    ...
    busta b;
    busta mailbox[N];
    sem_t mb_piena_sem, pool_buste_sem, m;
    T msg;
}gestore;
...
void myInit(struct gestore_t *g)
{
    ...
    //sem_init(&g->mb_vuota_sem, 0, N);
    sem_init(&g->mb_piena_sem, 0, 0);
...
void *send(struct gestore_t *g) {
    while (i) {
        ...
        if (&g->mb_liberi <= 0) {
            ...
            g->mb_mit++;
            sem_post(&g->m);
            sem_wait(&g->mb_piena_sem); // receiver deve fare post
        } else {
           // sem_wait(&g->mb_piena_sem);
            ...
            g->mb_liberi --;
            sem_post(&g->m);
            sem_post(&g->mb_piena_sem); // post deve fare wait
        }
    }
}
